...
<head>
  ...
  <title>Dashboard brands - RockStars</title>
</head>
    ...
    <aside class="inset-y-0 z-10 flex flex-col flex-shrink-0 w-fit max-h-screen overflow-hidden transition-all transform bg-white border-r shadow-lg lg:z-auto lg:static lg:shadow-none">
      ...
      <nav class="flex-1 overflow-hidden hover:overflow-y-auto">
        <ul class="p-2 overflow-hidden">
          ...
          <li>
            <a href="brands.php" class="flex justify-start items-center p-2 space-x-2 rounded-md bg-gray-100">
              <span>
                <svg class="w-6 h-6 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    <div class="flex flex-col flex-1 h-full overflow-hidden">
      ...
      <main class="flex-1 max-h-full p-5 overflow-hidden overflow-y-scroll">
        ...
        <div class="flex flex-col items-start justify-between pb-6 space-y-4 border-b lg:items-center lg:space-y-0 lg:flex-row">
          <h1 class="text-2xl font-semibold whitespace-nowrap">Manage Brands</h1>
        </div>

        <!-- Table see (https://tailwindui.com/components/application-ui/lists/tables) -->
        <div class="flex mt-6 items-center justify-between">
          <h3 class="text-xl">All brands (<?= $countBrands ?>)</h3>
                ...
                <table class="min-w-full overflow-x-scroll divide-y divide-gray-200">
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    foreach ($brands as  $brand) { ?>
                      <tr class="transition-all hover:bg-gray-100 hover:shadow-lg" onclick="getBrand(<?= $brand['id'] ?>)" id="<?= $brand['id'] ?>">
                        ...
                        <td class="brand px-6 py-4 whitespace-nowrap">
                          <p class="name text-sm truncate max-w-sm text-gray-900" brand="<?= $brand['name'] ?>"><?= $brand['name'] ?></p>
                        </td>
                        <td class="brand px-6 py-4 whitespace-nowrap">
                          <p class="description text-sm truncate max-w-sm text-gray-900" brand="<?= $brand['description'] ?>"><?= $brand['description'] ?></p>
                        </td>
                      </tr>
                    <?php } ?>
